...
static s64 ntfs_device_unix_io_seek(struct ntfs_device *dev, s64 offset,
		int whence)
{
	//return lseek(DEV_FD(dev), offset, whence); //wonfee
	return lseek64(DEV_FD(dev), offset, whence); //wonfee
}
...
{
	//return pread(DEV_FD(dev), buf, count, offset); //wonfee
		...
		s64 count, s64 offset)
{
	...
	NDevSetDirty(dev);
	//return pwrite(DEV_FD(dev), buf, count, offset); //wonfee
